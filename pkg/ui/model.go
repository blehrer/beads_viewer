...
func (m *Model) updateViewportContent() {
	...
	md := m.buildListDetailMarkdown(issueItem, item)
...
// buildListDetailMarkdown assembles markdown for the list/detail viewport (glamour input).
// ponytail: must stay ANSI-free — use *IconMD helpers, never RenderPriorityIcon/GetStatusIcon.
func (m *Model) buildListDetailMarkdown(issueItem IssueItem, item model.Issue) string {
	var sb strings.Builder
	...
	sb.WriteString(fmt.Sprintf("| **%s** | **%s** | %s | @%s | %s |\n\n",
		...
		GetPriorityIconMD(item.Priority),
		item.Assignee,
	...
	if m.semanticSearchEnabled && m.semanticHybridEnabled && issueItem.SearchScoreSet && m.list.FilterState() != list.Unfiltered {
		...
		if len(issueItem.SearchComponents) > 0 {
			...
			for _, key := range []string{"pagerank", "status", "impact", "priority", "recency"} {
	...
	if len(item.Dependencies) > 0 {
		rootNode := BuildDependencyTree(item.ID, m.issueMap, 3)
	...
	if m.historyView.HasReport() {
		if historyMD := m.renderBeadHistoryMD(item.ID); historyMD != "" {
